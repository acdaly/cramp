...
function setup() {
  ...
  setInterval(function(){ createClots(1, window.width/2 - Math.random()*100, window.height/2 - Math.random()*100); }, 500);
  ...
  createPills(PILL_NUM, pillX, pillY);
...
function drawShapes() {
  ...
  for (var b = 0; b < clots.length; b++){
    //for (var c = 0; c < clots[b].bodies.length; c++){
      //print(clots[b].bodies[c].vertices)
      drawShape(clots[b].body, red)
    //}
...
function checkPillCollisions(){
  ...
  for (var p = 0; p < pills.length; p++){
    for (var b = 0; b < clots.length; b++){
      ...
      var collision = Matter.SAT.collides(clots[b].body, pills[p].body);
      if (collision.collided) {
        print("clot collide!")
        // Matter.Composite.remove(engine.world, clots[b].bodies[c].body);
        clots.splice(b, 1);
      }
